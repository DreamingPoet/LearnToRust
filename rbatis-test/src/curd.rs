...
impl_select!(BizActivity{select_all_by_name_and_id(id:&str,name:&str) => "`where id = #{id} and name = #{name}`"});
impl_select!(BizActivity{select_by_id(id:&str) -> Option => "`where id = #{id} limit 1`"});
impl_select!(BizActivity{select_by_name(name:&str) => "`where name = #{name} limit 1`"});
...
pub async fn main() {
    ...
    // 
    let data = BizActivity::select_all_by_name_and_id(&mut rb, "2", "zxx1").await;
    println!("select_all_by_id = {:?}", data);
    if let Ok(res) = data {
      for i in res {
        println!("name = {}", i.name.unwrap());
      }
    }

    let data = BizActivity::select_by_id(&mut rb, "1").await;
    if let Ok(Some(ref res)) = data {
      println!("data = {:?}", res);
    }
    ...
    let data = BizActivity::select_by_name(&mut rb, "zxx1").await;
    println!("select_by_name = {:?}", data);
